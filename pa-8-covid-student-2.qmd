...
cases <- read_csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv") #reading in the cases data set
```
...
cases |> 
  filter(state == "California") |> #filter to CA
  mutate(date = date(date), #using the date within raw data
         month = month(date, label = TRUE, abbr = FALSE), #using the month of the date
         year = year(date) #using the year of the date 
         ) |> 
  unite(Month, "month", "year" , sep = "-") |> #uniting the month and year columns
  group_by(Month) |> #using Months
  summarize(Total_Cases = sum(cases_daily)) |> #finding sum of cases_daily
  ggplot() + #creating plot
    geom_col(mapping = aes(y = fct_reorder(Month,my(Month)),
                           x = Total_Cases,
                           ))
```
...
cases |> 
  filter(state == "California",
         format(date, "%Y") == "2021") |> #filter to CA
  mutate(date = date(date), #using the date within raw data
         month = month(date, label = TRUE, abbr = FALSE), #using the month of the date
         year = year(date) #using the year of the date 
         ) |> 
  ...
  labs(title="Covid Cases in California in 2021",
       subtitle="")
```
...
cases |> 
  filter(state == "California",
         format(date, "%Y") == "2020") |> #filter to CA
  mutate(date = date(date), #using the date within raw data
         month = month(date, label = TRUE, abbr = FALSE), #using the month of the date
         year = year(date) #using the year of the date 
         ) |> 
  ...
  labs(title="Covid Cases in California in 2020",
       subtitle="Which months showed the initial spike in COVID cases in CA, 2020?") 
  ggsave("COVID2020.png")
```
